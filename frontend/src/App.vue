...
const email = ref('admin@example.com'); const password = ref(''); const loginError = ref(''); const loggingIn = ref(false);
...
<template>
  ...
  <div v-else class="workspace">
    ...
    <aside class="sidebar" :class="{ opened: mobileNav }">
      ...
      <div class="sidebar-bottom"><div class="network-note"><ShieldCheck :size="19" /><div>推理接口地址<small>{{ overview?.internal_base_url || '—' }}</small></div></div><button class="profile" @click="navigate('settings')"><span class="avatar">A</span><span>Administrator<small>工作空间管理员</small></span><Settings2 :size="17" /></button></div>
    </aside>
    <div class="workspace-main">
      ...
      <main class="page-content">
        ...
        <template v-else-if="page === 'clients'">
          <section class="endpoint-card"><span class="endpoint-icon"><Globe2 :size="23" /></span><div><span class="eyebrow">推理地址 · OPENAI / ANTHROPIC</span><code>{{ overview?.internal_base_url || '未配置' }}</code></div><button class="button secondary" :disabled="!overview?.internal_base_url" @click="copy(overview!.internal_base_url)"><Copy :size="15" />复制地址</button></section><div class="info-strip"><ShieldCheck :size="18" /><span>原始密钥仅在创建或轮换时显示一次。数据库仅保存加盐哈希，不提供密钥找回。</span></div>
