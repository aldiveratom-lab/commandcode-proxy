...
export class Repository {
  ...
  billingCookie(id) {
    const row = this.db.prepare('SELECT credential FROM billing_sessions WHERE upstream_id=?').get(id);
    return row ? decrypt(row.credential, this.masterKey, `billing:${id}`) : null;
  }
  setBillingCookie(id, cookie) {
    this.db.prepare('INSERT OR REPLACE INTO billing_sessions VALUES(?,?)').run(id, encrypt(cookie, this.masterKey, `billing:${id}`));
    const row = this.get('upstreams', id);
    this.save('upstreams', { ...row, billing_authorized: true, billing: null, billing_error: null, billing_checked_at: null });
  }
  clearBilling(id) {
    this.db.prepare('DELETE FROM billing_sessions WHERE upstream_id=?').run(id);
    const row = this.get('upstreams', id);
    if (row) this.save('upstreams', { ...row, billing_authorized: false, billing: null, billing_error: null, billing_checked_at: null });
  }
