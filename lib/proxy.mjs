...
function httpConnect(proxy, target, signal) {
  return new Promise((resolve, reject) => {
    ...
    const req = transport.request({ host: proxy.host, port: proxy.port, method: 'CONNECT', path: target,
      headers: { Host: target, ...(auth ? { 'Proxy-Authorization': auth } : {}) }, timeout: CONNECT_TIMEOUT_MS, agent: false });
...
export async function proxyFetch(urlStr, options = {}, rawProxy) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = transport.request({ host: u.hostname, port, path: u.pathname + u.search, method: options.method || 'GET', headers: options.headers || {}, createConnection: () => socket, agent: false }, res => {
