...
import { BillingService, validateBillingCookie } from './billing.mjs';
...
export async function createApplication({ databasePath = ':memory:', masterKey, initialPassword, origin = 'https://sub.sunmmyapi.xyz', apiBase = 'https://api.commandcode.ai', assetsPath = resolve('frontend/dist'), maxInflight = 4, maxBodyBytes = 8 * 1024 * 1024, trustProxy = false } = {}) {
  ...
  const management = protect(async (req, res, url) => {
    ...
    const match = path.match(/^\/command\/api\/(upstreams|clients)(?:\/([a-zA-Z0-9-]+))?(?:\/(test|models\/refresh|rotate|billing\/refresh|billing\/session))?$/);
    ...
    if (!id) {
      if (method === 'GET') return json(res, 200, { data: repo.list(kind).map(a => kind === 'upstreams' ? scheduler.view(a) : a) });
      if (method === 'POST') {
        const body = await bodyJSON(req); const value = fields(body, kind);
        const result = kind === 'upstreams' ? repo.createUpstream(value, body.credential) : repo.createClient(value);
    ...
    if (kind === 'upstreams' && action === 'billing/session') {
      if (method === 'PUT') {
        const body = await bodyJSON(req, 16384);
        if (body.confirm_account !== true) fail(400, 'invalid_input', '请确认登录会话属于所选上游账号');
        repo.setBillingCookie(id, validateBillingCookie(body.cookie));
        repo.audit('upstreams.billing.authorize', id, 'success', ip);
        return json(res, 200, scheduler.view(repo.get(kind, id)));
      }
      if (method === 'DELETE') {
        repo.clearBilling(id); repo.audit('upstreams.billing.revoke', id, 'success', ip);
        return json(res, 200, scheduler.view(repo.get(kind, id)));
      }
    }
    if (kind === 'upstreams' && action === 'billing/refresh' && method === 'POST') {
      if (active >= maxInflight) fail(503, 'server_busy', '服务繁忙，请稍后刷新', 5);
      active++;
      try { return json(res, 200, scheduler.view(await billing.refresh(id))); }
      finally { active--; }
    }
    if (!action) {
      if (method === 'GET') return json(res, 200, kind === 'upstreams' ? scheduler.view(row) : row);
      if (method === 'PATCH') {
        ...
        return json(res, 200, kind === 'upstreams' ? scheduler.view(value) : value);
      }
      ...
      if (kind === 'upstreams' && (action === 'models/refresh' || action === 'test')) {
        ...
        try {
          if (action === 'models/refresh') {
            ...
            try { const result = await upstream.refresh(id, abort.signal); repo.audit('upstreams.models', id, 'success', ip); return json(res, 200, result); }
  ...
  const inference = protect(async (req, res, url) => {
    ...
    active++;
    const abort = new AbortController(); const close = () => { if (!res.writableEnded) abort.abort(); }; res.once('close', close);
    let lease; let success = false; const started = Date.now(); const context = { apiBase, signal: abort.signal, status: null, dispatched: false };
    try {
      ...
      if (lease && context.dispatched) scheduler.result(lease.account.id, { status: success ? 200 : context.status >= 400 ? context.status : res.statusCode >= 400 ? res.statusCode : 0, retryAfter: context.retryAfter, cancelled: abort.signal.aborted });
